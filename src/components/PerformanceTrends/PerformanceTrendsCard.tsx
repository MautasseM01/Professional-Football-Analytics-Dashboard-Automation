...
export const PerformanceTrendsCard = ({
  ...
  const {
    ...
    loading,
    error
  } = usePlayerMatchPerformance(player, undefined, 15);
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      return <div className={cn("border rounded-xl shadow-2xl backdrop-blur-md p-3 transition-all duration-200", theme === 'dark' ? "bg-club-dark-gray/95 border-club-gold/30 text-club-light-gray" : "bg-white/95 border-club-gold/40 text-gray-900")}>
          <p className="font-semibold text-club-gold text-sm">{data.match}</p>
          <p className={cn("text-xs mb-1", theme === 'dark' ? "text-club-light-gray/80" : "text-gray-600")}>{data.date}</p>
          <p className="text-club-gold font-medium text-sm">
            {selectedKPIOption?.label}: {payload[0].value}{selectedKPIOption?.unit}
          </p>
          {showMovingAverage && data.movingAvg !== null && <p className={cn("text-xs", theme === 'dark' ? "text-gray-400" : "text-gray-500")}>
  ...
  if (loading) {
    return <Card className={cn("border-club-gold/20", theme === 'dark' ? "bg-club-dark-gray" : "bg-white")}>
        <CardHeader>
  ...
  if (error) {
    return <Card className={cn("border-club-gold/20", theme === 'dark' ? "bg-club-dark-gray" : "bg-white")}>
        ...
        <CardContent className="p-6">
          <ErrorFallback title="Performance trends error" description={`Failed to load performance trends: ${error}`} />
        </CardContent>
      </Card>;
  }
  ...
  return <Card className={cn("border-club-gold/20 transition-all duration-300", theme === 'dark' ? "bg-club-dark-gray hover:bg-club-dark-gray/80" : "bg-white hover:bg-gray-50")}>
      <CardHeader>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedKPI} onValueChange={setSelectedKPI}>
              ...
              <SelectContent className={cn("z-50 backdrop-blur-md border-club-gold/30", theme === 'dark' ? "bg-club-dark-gray/95 text-club-light-gray" : "bg-white/95 text-gray-900")}>
                ...
                return <SelectItem key={option.value} value={option.value} className={cn("flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-150", "focus:bg-club-gold/20 hover:bg-club-gold/10", selectedKPI === option.value && "bg-club-gold/20 text-club-gold")}>
                      <div className="flex items-center gap-3">
                        <IconComponent className={cn("w-4 h-4", selectedKPI === option.value ? "text-club-gold" : "text-gray-500")} />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <Switch id="showStatistics" checked={showStatistics} onCheckedChange={setShowStatistics} className="data-[state=checked]:bg-club-gold" />
            </div>
        ...
        {/* Statistics Display */}
        {showStatistics && <div className={cn("grid gap-4 p-4 rounded-2xl transition-all duration-300", isMobile ? "grid-cols-2" : "grid-cols-4", theme === 'dark' ? "bg-club-black/30 border border-club-gold/10" : "bg-gray-50/50 border border-club-gold/20")}>
            <div className="text-center">
              <div className="text-lg font-bold text-club-gold">
                {stats.avg}{selectedKPIOption?.unit}
              </div>
              <div className={cn("text-xs", theme === 'dark' ? "text-club-light-gray/70" : "text-gray-600")}>Average</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-green-500">
                {stats.max}{selectedKPIOption?.unit}
              </div>
              <div className={cn("text-xs", theme === 'dark' ? "text-club-light-gray/70" : "text-gray-600")}>Best</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-red-500">
                {stats.min}{selectedKPIOption?.unit}
              </div>
              <div className={cn("text-xs", theme === 'dark' ? "text-club-light-gray/70" : "text-gray-600")}>Worst</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-blue-500">
                {chartData.length}
              </div>
              <div className={cn("text-xs", theme === 'dark' ? "text-club-light-gray/70" : "text-gray-600")}>Matches</div>
            </div>
          </div>}
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            {chartView === "area" ? <AreaChart data={chartData}>
                <defs>
              ...
              fontSize: 12
                ...
                <Area type="monotone" dataKey="value" stroke="#D4AF37" strokeWidth={2} fill="url(#metricGradient)" dot={{
              fill: "#D4AF37",
              strokeWidth: 2,
              r: 4
            }} />
              ...
              fontSize: 12
                ...
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="value" stroke="#D4AF37" strokeWidth={2} dot={{
              fill: "#D4AF37",
              strokeWidth: 2,
              r: 4
            }} />
