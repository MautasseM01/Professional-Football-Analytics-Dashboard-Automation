...
export const DesktopControls = ({
  ...
  return <>
      ...
      <div className={cn("grid gap-3 sm:gap-4", isMobile ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3")}>
        <div className="space-y-2 px-[16px] py-0">
          <Label className={cn("text-sm font-medium", theme === 'dark' ? "text-club-light-gray/80" : "text-gray-600")}>Performance Metric</Label>
          <Select value={selectedKPI} onValueChange={setSelectedKPI}>
            <SelectTrigger className={cn("w-full border-club-gold/30 focus:ring-club-gold/50 text-sm rounded-xl transition-all duration-200", isMobile ? "h-11" : "h-10", theme === 'dark' ? "bg-club-black/50 text-club-light-gray" : "bg-white/70 text-gray-900")}>
              <SelectValue placeholder="Select KPI" />
            </SelectTrigger>
            <SelectContent className={cn("border-club-gold/30 z-50 max-h-60 rounded-xl backdrop-blur-md", theme === 'dark' ? "bg-club-black/90 text-club-light-gray" : "bg-white/90 text-gray-900")}>
              {KPI_OPTIONS.map(option => <SelectItem key={option.value} value={option.value} className={cn("focus:bg-club-gold/20 rounded-lg transition-colors duration-150", isMobile ? "text-base py-3" : "text-sm")}>
                  {option.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2 px-[16px] py-0">
          <Label className={cn("text-sm font-medium", theme === 'dark' ? "text-club-light-gray/80" : "text-gray-600")}>Time Period</Label>
          <Select value={selectedTimePeriod} onValueChange={setSelectedTimePeriod}>
            <SelectTrigger className={cn("w-full border-club-gold/30 focus:ring-club-gold/50 text-sm rounded-xl transition-all duration-200", isMobile ? "h-11" : "h-10", theme === 'dark' ? "bg-club-black/50 text-club-light-gray" : "bg-white/70 text-gray-900")}>
              <SelectValue placeholder="Time Period" />
            </SelectTrigger>
            <SelectContent className={cn("border-club-gold/30 z-50 rounded-xl backdrop-blur-md", theme === 'dark' ? "bg-club-black/90 text-club-light-gray" : "bg-white/90 text-gray-900")}>
              {TIME_PERIOD_OPTIONS.map(option => <SelectItem key={option.value} value={option.value} className={cn("focus:bg-club-gold/20 rounded-lg transition-colors duration-150", isMobile ? "text-base py-3" : "text-sm")}>
                  {option.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2 px-[16px] py-0">
          <Label className={cn("text-sm font-medium", theme === 'dark' ? "text-club-light-gray/80" : "text-gray-600")}>Chart View</Label>
          <Select value={chartView} onValueChange={setChartView}>
            <SelectTrigger className={cn("w-full border-club-gold/30 focus:ring-club-gold/50 text-sm rounded-xl transition-all duration-200", isMobile ? "h-11" : "h-10", theme === 'dark' ? "bg-club-black/50 text-club-light-gray" : "bg-white/70 text-gray-900")}>
              <SelectValue placeholder="Chart View" />
            </SelectTrigger>
            <SelectContent className={cn("border-club-gold/30 z-50 rounded-xl backdrop-blur-md", theme === 'dark' ? "bg-club-black/90 text-club-light-gray" : "bg-white/90 text-gray-900")}>
              {CHART_VIEW_OPTIONS.map(option => <SelectItem key={option.value} value={option.value} className={cn("focus:bg-club-gold/20 rounded-lg transition-colors duration-150", isMobile ? "text-base py-3" : "text-sm")}>
                  {option.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
