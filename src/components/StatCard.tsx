...
export const StatCard = ({
  ...
  return <Card className={`
      ...
      ${className}
    `}>
      <CardContent className="p-4 sm:p-5 lg:p-6 h-full flex flex-col justify-between min-h-[100px] px-[24px]">
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div className="text-sm text-gray-300 light:text-gray-700 font-medium leading-tight min-w-0 flex-1">
              {title}
            </div>
            {icon && <div className="text-club-gold/50 light:text-yellow-600/60 flex-shrink-0 group-hover:text-club-gold light:group-hover:text-yellow-600 transition-colors duration-300">
          ...
          <div className="space-y-1">
            ...
            {subValue && <div className="text-xs text-gray-400 light:text-gray-600 break-words leading-tight">
                {subValue}
              </div>}
          </div>
        </div>
