      ...
      <CardContent className="p-4 pt-0">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-club-dark-gray h-8">
            <TabsTrigger 
              value="overview" 
              className="data-[state=active]:bg-club-gold data-[state=active]:text-club-black text-xs h-6"
            >
          ...
          <TabsContent value="overview" className="mt-3">
            <div className="flex items-center justify-between gap-4 text-sm">
              <div className="flex items-center gap-2">
                <span className="text-club-light-gray/70">Total Cards:</span>
                <span className="text-club-gold font-medium">{disciplinaryData.totalCards}</span>
                <div className="flex gap-1">
                  {Array.from({ length: disciplinaryData.yellowCards }).map((_, i) => (
                    <div key={`yellow-${i}`} className="bg-[#FCD34D] text-black text-xs font-bold w-4 h-5 rounded-sm flex items-center justify-center border border-gray-300">
                      Y
                    </div>
                  ))}
                  {Array.from({ length: disciplinaryData.redCards }).map((_, i) => (
                    <div key={`red-${i}`} className="bg-[#EF4444] text-black text-xs font-bold w-4 h-5 rounded-sm flex items-center justify-center border border-gray-300">
                      R
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-4 text-xs">
                <div>
                  <span className="text-club-light-gray/70">Fair Play:</span>
                  <span className="text-club-gold font-medium ml-1">{disciplinaryData.fairPlayRating}/10</span>
                </div>
                <div>
                  <span className="text-club-light-gray/70">Missed:</span>
                  <span className="text-club-gold font-medium ml-1">{disciplinaryData.missedMatches}</span>
                </div>
              </div>
            </div>
          </TabsContent>
