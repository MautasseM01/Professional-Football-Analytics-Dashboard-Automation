...
export const DashboardSidebar = () => {
  ...
  if (isMobile && mobileOpen) {
    return <>
        ...
        <div className="fixed inset-0 bg-club-black/95 backdrop-blur-md z-50 lg:hidden transition-all duration-300 ease-in-out" onClick={closeMobileMenu}>
          {/* Full-screen mobile menu content with proper scrolling */}
          <div className="h-full w-full flex flex-col relative" onClick={e => e.stopPropagation()}>
            {/* Close button - large touch target in top-right */}
            <div className="flex justify-end p-6 flex-shrink-0 px-[16px] py-0">
              <Button variant="ghost" size="icon" onClick={closeMobileMenu} className="text-club-gold hover:text-club-gold/80 hover:bg-club-gold/10 min-h-[48px] min-w-[48px] touch-manipulation rounded-xl" aria-label="Close menu">
                <X size={28} />
              </Button>
            </div>
            ...
            <ScrollArea className="flex-1 px-6">
              <div className="flex flex-col items-center pb-8 rounded-md">
                {/* Logo and branding */}
                <div className="flex flex-col items-center mb-12">
                  <img src="/lovable-uploads/eb223be6-87a6-402c-a270-20313a00080c.png" alt="Club Logo" className="w-16 h-16 mb-4 rounded-xl shadow-lg" />
                ...
                <div className="flex flex-col items-center space-y-3 w-full max-w-sm">
                  {filteredNavigationItems.map(item => <div key={item.name} className="w-full">
                      <SidebarNavItem item={item} collapsed={false} openSubMenu={openSubMenus.has(item.name) ? item.name : null} toggleSubMenu={toggleSubMenu} onNavigate={handleNavigate} className="min-h-[48px] text-lg justify-center text-center hover:bg-club-gold/10 rounded-xl touch-manipulation py-3 px-6 transition-all duration-200 ease-in-out" />
                    </div>)}
  ...
  return <>
      ...
      {isMobile && <Button variant="ghost" size="icon" onClick={toggleMobileMenu} className="fixed top-4 left-4 z-30 text-club-gold hover:text-club-gold/80 hover:bg-club-gold/10 bg-club-black/90 backdrop-blur-sm lg:hidden min-h-[48px] min-w-[48px] touch-manipulation shadow-lg rounded-xl border border-club-gold/20" aria-label="Open navigation menu">
          <Menu size={24} />
        </Button>}
